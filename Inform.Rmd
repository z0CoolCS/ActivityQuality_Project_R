...
ggplot(data = datos_long, aes(x = variable, y = X, fill = ausente)) +
  ...
  theme_bw() +
  labs(title = "NA values for variable") +
...
```
Additionally, I noticed the first seven variables do not offer any important information because they are string or incremental variables.  
```{r}
str(training)
```
So, we omit columns from one to seven.  
```{r}
...
Another fact, it find which variables are highly correlated. If we can find them, we can omit some of them in order to get loss computational load but still have a good accuracy. For that purpose, we use *corrplot* with order set to hclust to see some possible omissions in our analysis.  
...
Later, in order to scale with our methodology, I will use only variable which give me 95\% of explanation about the data with PCA.  
```{r}
set.seed(123)
...
My first intuition is using SVM with kernelization. 
```{r}
set.seed(123)

control_train <- trainControl(method = "repeatedcv", number = 3,
                              repeats = 2, returnResamp = "final", verboseIter = FALSE,allowParallel = TRUE)
...
dev.off()
```
Getting `r confusion_matrix_svm$overall[1]` of precision with the valid. 

![Confusion Matrix of SVM](cm_svm.png)

### Decision Tree
In order to compere with another machine learning algorithm, I decided to used decision tree. 
```{r}
control_train <- trainControl(method = "repeatedcv", number = 3, repeats = 2, returnResamp = "final", verboseIter = FALSE, allowParallel = TRUE)
...
```
Unfortunately, I only got `r confusion_matrix_dt$overall[1]` of precision.  

![Confusion Matrix of Decision Tree.](cm_dt.png)

### Random Forest
After using decision tree, I tried with random forest. 
...
```
Fortunately, random forest gave me `r confusion_matrix_rf$overall[1]` of precision, so I took it to predict with test data.
![Confusion Matrix of Random Forest.](cm_rf.png)

## Results
Finally, I used random forest to get the answers for the testing data.  
```{r}
dim(testing)
testing_pc <- predict(pp_pca, testing[,-53])
predict(model_rf, testing_pc)
```
